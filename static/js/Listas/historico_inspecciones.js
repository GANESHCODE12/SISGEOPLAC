...
function generate_report() {
  ...
  tblHistoricoInspecciones = $('#data').DataTable({
    responsive: true,
    autoWidth: false,
    destroy: true,
    order: [0, 'desc'],
  ...
  $('#data tbody')
    .on('click', 'a[rel=details_pnc]', function () {
      var tr = tblHistoricoInspecciones.cell($(this).closest('td, li')).index();
      var data = tblHistoricoInspecciones.row(tr.row).data();
