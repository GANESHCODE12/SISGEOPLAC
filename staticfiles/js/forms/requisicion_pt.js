...
function formatRepo(repo) {
    ...
    var option = $(
        ...
        '<b>Código:</b> <span class="badge badge-warning">'+repo.codigo_producto+'</span>'+
        '</p>' +
        '</div>' +
        '</div>' +
        '</div>');
...
$(function () {
    ...
    $('#tblRequisionesPT tbody').on('change', 'input[name="cantidad_solicitada"]', function () {
        ...
        $('td:eq(4)', tblRequisionesPT.row(tr.row).node()).html(solicitudes_pt.items.requisicion_pt[tr.row].cantidad_ingresada.toFixed(2))
    })
